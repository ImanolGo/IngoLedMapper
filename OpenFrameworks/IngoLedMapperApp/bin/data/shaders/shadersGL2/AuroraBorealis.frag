...
void main(void) {
    ...
    uv*= texResolution/iResolution.xy;
    vec2 pos =  uv * 0.25 + vec2(0.0, iTime * 0.025);
   //pos*= texResolution/iResolution.xy;
    
    float o = texture2DRect(iChannel1, pos).r;

    pos = uv * 0.25 - vec2(0.0, iTime * 0.02 + o * 0.02);
   // pos*= texResolution/iResolution.xy;
    float d = (texture2DRect(iChannel0, pos).r * 2.0 - 1.0);
