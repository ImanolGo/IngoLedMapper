...
const int LedsManager::LEDS_PER_STRIPE = 33;
...
void LedsManager::readLedsPosition()
{
    ...
    if(buffer.size())
    {
        ...
        for (ofBuffer::Line it = buffer.getLines().begin(), end = buffer.getLines().end(); it != end; ++it)
        {
            ...
            {
                int i = id/LEDS_PER_STRIPE;
                int j = id%LEDS_PER_STRIPE;
                ofPoint mapPos(i,j,0);
                createLed(ledPosition, mapPos, id);
                id++;
            }
...
void LedsManager::centreLeds()
{
    ...
    for (auto led: m_leds)
    {
        auto position = led->getMapPosition();
...
void LedsManager::createLed(const ofPoint& position, const ofPoint& map_position, int& id)
{
    ofPtr<Led> led = ofPtr<Led> (new Led ( position, map_position, id ) );
    float size = AppManager::getInstance().getGuiManager().getLedsSize();
    led->setWidth(size);
    m_leds.push_back(led);
    
    ofLogNotice() <<"LedsManager::createLed -> id " << led->getId() << ", x = "  << led->getMapPosition().x << ", y = "  << led->getMapPosition().y << ", z = " << led->getMapPosition().z ;
}
...
{
    string path = "leds/Schuster_3D_points.csv";
    this->load(path);
}
